...
from jaxonlayers.layers.attention import MultiheadAttention

RUN_DESCRIPTION = "Using self-learned default embeddings + dropout + batch_norm + aggressive dropout usage + larger network"


class FrequencyAttention(eqx.Module):
    attention: MultiheadAttention

    def __init__(self, key: jt.PRNGKeyArray):
        self.attention = MultiheadAttention(
            embed_dim=2, num_heads=1, dropout=0.0, inference=True, key=key
        )

    def __call__(self, x: jt.Array) -> jt.Array:
        freq = jnp.fft.fft(x)

        freq_stacked = jnp.stack([freq.real, freq.imag], axis=-1)

        attn_output, _ = self.attention(
            query=freq_stacked, key=freq_stacked, value=freq_stacked, need_weights=False
        )

        freq_complex = attn_output[..., 0] + 1j * attn_output[..., 1]

        output = jnp.fft.ifft(freq_complex).real
...
class SimpleModel(eqx.Module):
    ...
    freq_attention: FrequencyAttention
    mlp: eqx.nn.MLP
    ...
    def __init__(
        ...
        self.freq_attention = FrequencyAttention(key=k5)
        self.mlp = eqx.nn.MLP(
    ...
    def __call__(
        ...
        x_transposed = x.T
        x_processed = eqx.filter_vmap(self.freq_attention)(x_transposed)
        x = x_processed.T
